...
import { GetIssues, ReturnBook,DeleteIssue } from '../../../apicalls/issues';
...
function Issues({ open = false, setOpen, selectedBook, reloadBooks }) {
    ...
    const onReturnHandler = async (issue) => {
        try {
            ...
            dispatch(HideLoading());
            if (response.success) {
                setIssues(prevIssues => prevIssues.filter(item => item._id !== issue._id));
                // getIssues();
                reloadBooks();
            } else {
    ...
    const deleteIssueHandler = async (issue) => {
        try {
            ...
            dispatch(HideLoading());
            if (response.success) {
                setIssues(prevIssues => prevIssues.filter(item => item._id !== response.data._id));
                getIssues();
                reloadBooks();
            } else {
                message.error(response.message);
